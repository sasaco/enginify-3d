...
export class IfcAPI {
    ...
    */
    ...
    SetWasmPath(path: string, absolute = false) {
        // Clean any credentials from the path if it's a URL
        if (path.includes('@')) {
            try {
                ...
                // If path is not a valid URL, use it as-is
            }
        }
        this.wasmPath = path;
        this.isWasmPathAbsolute = absolute || path.startsWith("http://") || path.startsWith("https://") || path.startsWith("file://");
    }
